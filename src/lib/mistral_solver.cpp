...
void Mistral::Solver::treat_bound_literal2(Literal q){
...
void Mistral::Solver::treat_explanation3 (Explanation* explanation,  Explanation::iterator start,Explanation::iterator end ){
	//Literal q;
	while(start < end) {
		//q = *start;
		//++start;
			...
			if(current_explanation == NULL) {
				...
				if(_DEBUG_FD_NOGOOD){
					...
					else
					{
						...
						std::cout << " \n \n \n we will explain the boolean variable " << variables[a_literal] << " ; its domain : " << variables[a_literal].get_domain() << " its assignment_level : " << assignment_level[a_literal] << std::endl;
					}
					std::cout << " this action (i.e. its explanation) comes from : "<< current_explanation << std::endl;
				}
...
#ifdef 	_DEBUG_FD_NOGOOD
					if(_DEBUG_FD_NOGOOD){
//						std::cout << " \n learn :  " << variables[a_literal] << "  = " <<  variables[a].get_domain() << " ; assignment_level : " << assignment_level[a]<< std::endl;
					}
#endif
#ifdef _DEBUG_SEARCH
					if(_DEBUG_SEARCH) {
...
int Mistral::Solver::reduce_bounds(){
	...
	for (int i = size_bitset; i>0; --i ){
		...
		if (lazygeneration){
			...
			id_bool = -1;
			//	if (!is_lb)
			//		id_bool = dom_constraint->value_exist( val ) ;
			//	else
...
//		std::cout << "\n explanation" << *explanation <<std::endl;
	...
	//	start_tmp_iterator = explanation->get_reason_for_literal(tmp_literal, end_tmp_iterator);
		...
		if(!remove)
			visitedLowerBounds.fast_add(visited_var);
		else{
			++reduced;
		//	std::cout << "removed X" << visited_var << " >= " << visited_val+1 << std::endl;
		}
	...
	for (int i = size_bitset ; i>0; --i ){
		...
		if (lazygeneration){
			dom_constraint = tmp_VariableRangeWithLearning->domainConstraint;
			id_bool = -1;
			//We know that it is an Upper bound
			//	if (!is_lb)
	...
	//	start_tmp_iterator = explanation->get_reason_for_literal(tmp_literal, end_tmp_iterator);
		...
		if(!remove)
			visitedUpperBounds.fast_add(visited_var);
		else{
			++reduced;
	//		std::cout << "removed X" << visited_var << " <= " << visited_val << std::endl;
		}
...
*/
			...
			//	if (graph_size <35)
			{
				/*		Vector< Literal >  tmp_nogood;
				tmp_nogood.clear();
				//124 3026 135 1160 1939 1943 1164 1246 1187 2097 3314 3316 3508 3521]
			...
			if (is_a_bound_literal(tmp))
			{
				...
				if (lvl_bool>search_root){
						...
						//explored = false;
					{
						...
						//if (id_tmp>= initial_variablesize)
					{
						//Here semantic_reduction is true!
