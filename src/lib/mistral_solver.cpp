...
void Mistral::Solver::learn_with_lazygeneration_64bits_implementation()
{
	...
	std::cout << "NULL_ATOM   " << NULL_ATOM << std::endl;
		...
		std::cout << "  level "  << level << std::endl;
			...
			if(current_explanation == NULL) {
				...
				store_reason(current_explanation, NULL_ATOM);
#endif

				Explanation::iterator lit = current_explanation->get_reason_for(NULL_ATOM, level, stop);
				graph_size++;
				tmp = lit;
				bound_literals_to_explore.clear();
...
//							Explanation::iterator start_tmp_iterator = bound_explanation->get_reason_for(q, level, end_tmp_iterator);
						Explanation::iterator start_tmp_iterator = bound_explanation->get_reason_for_bound(q, end_tmp_iterator);
...
#else
											std::cout << " _CHECK_NOGOOD should be enabled " <<std::endl;
											exit(1);
#endif
...
//    	simple_fdlearn_nogood();
   ...
   // fdlearn_nogood_nosequence();
 	...
 	//learn_with_lazygeneration_64bits_implementation();
