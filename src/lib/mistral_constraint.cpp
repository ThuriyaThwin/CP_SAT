...
Mistral::PropagationOutcome Mistral::ExplainedConstraintLess::propagate() {
...
Mistral::Explanation::iterator Mistral::ExplainedConstraintLess::get_reason_for_bound(const Literal a, iterator& end){

#ifdef _VERIFY_BEHAVIOUR_WHEN_LEARNING
	if (!is_a_bound_literal(a)){
		std::cout <<" \n \n ERROR not a bound literal in get_reason_for_bound"  << std::endl;
		exit(1);
	}
#endif
	...
	if(a == NULL_ATOM){
		...
		if (! is_a_bound_literal(q1)){
			std::cout << " q1 ERROR in explaning failure on ExplainedConstraintLess" << std::endl;
			exit(1);
		}
		if (! is_a_bound_literal(q2)){
			std::cout << " q2 ERROR in explaning failure on ExplainedConstraintLess " << std::endl;
			exit(1);
		}
		...
		if (tmp__iterator < end){
			Literal q1= *tmp__iterator;
			if (! is_a_bound_literal(q1)){
				std::cout << " q1 explanation ERROR on ExplainedConstraintLess" << std::endl;
				exit(1);
			}
			if (! is_a_bound_literal(a)){
			...
			if (is_lower_bound(a) && is_upper_bound(q1) ){
				std::cout << " a&ub explanation ERROR on ExplainedConstraintLess" << std::endl;
				exit(1);
			}
			if (is_lower_bound(q1) && is_upper_bound(a) ){
				std::cout << " q1 && ub explanation ERROR on ExplainedConstraintLess" << std::endl;
				exit(1);
			}
			...
			if (level1> level2){
				std::cout << " lvl explanation ERROR on ExplainedConstraintLess" << std::endl;
				exit(1);
			}
		}
		else {
...
Mistral::Explanation::iterator Mistral::ExplainedConstraintReifiedDisjunctive::get_reason_for_bound(const Literal a, iterator& end){
...
*/
void Mistral::ConstraintReifiedDisjunctiveGlobal::initialise() {
...
void Mistral::ConstraintReifiedDisjunctiveGlobal::mark_domain() {
  get_solver()->forbid(scope[0].id(), LIST_VAR);
  get_solver()->forbid(scope[1].id(), LIST_VAR);
}

Mistral::PropagationOutcome Mistral::ConstraintReifiedDisjunctiveGlobal::propagate() {
  PropagationOutcome wiped = CONSISTENT;
  if( *min_t0_ptr + processing_time[0] > *max_t1_ptr ) {
...
Mistral::PropagationOutcome Mistral::ConstraintReifiedDisjunctiveGlobal::propagate(const int changed_idx, const Event evt) {
  PropagationOutcome wiped = CONSISTENT;
...
std::ostream& Mistral::ConstraintReifiedDisjunctiveGlobal::display(std::ostream& os) const {
  if(scope[2].is_ground()) {
    if(!scope[2].get_max()) {
      os << scope[1] << " + " << processing_time[1] << " <= " << scope[0]/*.get_var()*/ ;
    } else {
...
Mistral::Explanation::iterator Mistral::DomainFaithfulnessConstraint::get_reason_for_bound(const Literal a, iterator& end){
	...
	std::cout <<" \n \n ERROR : explain bound literal in Domain Faithfulness"  << std::endl;
	exit(1);
